...
def fetch_calls(
    ...
    .. code-block:: python
        ...
        # Please set the AP_TOKEN environment variable to your token
        calls_status, calls_df = AP_fetch.fetch_calls(
            ticker='SPY')
    ...
    """
    label = 'fetch_calls'
    datafeed_type = ap_consts.DATAFEED_AP_CALLS
    exp_date = None
    latest_pricing = {}
    latest_close = None
    ...
    use_url = ap_consts.AP_URLS['options'].format(
        ticker,
        exp_date)
    headers = ap_consts.get_auth_headers()
    ...
    if res.status_code != requests.codes.OK:
        if res.status_code in [401, 403]:
            log.critical(
                'Please check the AP_TOKEN is correct '
    ...
    for node in org_records:
        ...
        if (
            ...
            new_node = {}
            for col in ap_consts.AP_OPTION_COLUMNS:
                if col in node:
                    if col in ap_consts.AP_EPOCH_COLUMNS:
...
def fetch_puts(
    ...
    .. code-block:: python
        ...
        puts_status, puts_df = AP_fetch.fetch_puts(
            ticker='SPY')
    ...
    """
    label = 'fetch_calls'
    datafeed_type = ap_consts.DATAFEED_AP_PUTS
    exp_date = None
    latest_pricing = {}
    latest_close = None
