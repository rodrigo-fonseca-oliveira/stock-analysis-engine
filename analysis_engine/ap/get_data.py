...
import analysis_engine.ap.fetch_data as ap_fetch_data
...
def get_data_from_ap(
    ...
    try:
        ...
        ap_req = None
        if ft_type == ap_consts.FETCH_AP_CALLS or ft_str == 'apcalls':
            ft_type == ap_consts.FETCH_AP_CALLS
            ap_req = api_requests.build_ap_fetch_calls_request(
                label=label)
        elif ft_type == ap_consts.FETCH_AP_PUTS or ft_str == 'apputs':
            ft_type == ap_consts.FETCH_AP_PUTS
            ap_req = api_requests.build_ap_fetch_puts_request(
                label=label)
        else:
            log.error(
        ...
        if not ap_req:
            err = (
                f'{label} - ticker={ap_req["ticker"]} did not build a ap '
            ...
            log.debug(
                f'{label} - ticker={ap_req["ticker"]} field={field} '
                f'orient={orient} fetch')
        # if invalid ap request

        df = None
        try:
            if 'from' in work_dict:
                ap_req['from'] = datetime.datetime.strptime(
                    '%Y-%m-%d %H:%M:%S',
                    work_dict['from'])
            status_df, df = ap_fetch_data.fetch_data(
                work_dict=ap_req,
                fetch_type=ft_type)
            ...
            if status_df == ae_consts.SUCCESS:
                ...
                err = (
                    f'{label} - ticker={ap_req["ticker"]} '
                    f'ap_fetch_data.fetch_data field={ft_type} '
                    'failed fetch_data')
                log.critical(err)
                res = build_result.build_result(
                    status=ae_consts.ERR,
            ...
            err = (
                f'{label} - ticker={ap_req["ticker"]} field={ft_type} '
        ...
        if ae_consts.ev('DEBUG_AP_DATA', '0') == '1':
            log.debug(
                f'{label} ticker={ap_req["ticker"]} field={field} '
                f'data={rec["data"]} to_json')
        else:
            log.debug(
                f'{label} ticker={ap_req["ticker"]} field={field} to_json')
        # end of if/else found data

        upload_and_cache_req = copy.deepcopy(ap_req)
        ...
        try:
            ...
            err = (
                f'{label} - failed to upload ap data={upload_and_cache_req} '
        ...
        if not rec['data']:
            log.debug(
                f'{label} - ticker={ap_req["ticker"]} no Tradier data '
        ...
        res = build_result.build_result(
            status=ae_consts.ERR,
            err=(
                'failed - get_data_from_ap '
                f'dict={work_dict} with ex={e}'),
            rec=rec)
    # end of try/ex

    log.debug(
        'task - get_data_from_ap done - '
        f'{label} - status={ae_consts.get_status(res["status"])} '
        f'err={res["err"]}')

    return res
# end of get_data_from_ap
