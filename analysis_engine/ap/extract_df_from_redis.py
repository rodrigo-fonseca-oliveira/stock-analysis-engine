"""
Extract an APC dataset from Redis (S3 support coming soon) and
...
def extract_option_calls_dataset(
    ...
    Extract the Alpaca options calls for a ticker and
    return a tuple (status, ``pandas.Dataframe``)

    .. code-block:: python

        import analysis_engine.alpaca.extract_df_from_redis as ap_extract

        # extract by historical date is also supported as an arg
        # date='2019-02-15'
        calls_status, calls_df = ap_extract.extract_option_calls_dataset(
            ticker='SPY')
        print(calls_df)
    ...
    ds_id = ticker
    df_type = ap_consts.DATAFEED_APC_CALLS
    df_str = ap_consts.get_datafeed_str_ap(df_type=df_type)
    redis_db = ae_consts.REDIS_DB
    redis_key = f'{ticker}_{use_date}_apcalls'
    ...
    try:
        ...
        if status == ae_consts.SUCCESS:
            calls_json = None
            if 'apcalls' in redis_rec['rec']['data']:
                calls_json = redis_rec['rec']['data']['apcalls']
            ...
            try:
                ...
                if 'date' not in calls_df:
                    if verbose:
                        log.error(
                            'failed to find date column in APC calls '
...
def extract_option_puts_dataset(
    ...
    Extract the AP options puts for a ticker and
    return a tuple (status, ``pandas.Dataframe``)
    ...
    .. code-block:: python
        ...
        # extract by historical date is also supported as an arg
        # date='2019-02-15'
        puts_status, puts_df = ap_extract.extract_option_puts_dataset(
            ticker='SPY')
        print(puts_df)
    ...
    ds_id = ticker
    df_type = ap_consts.DATAFEED_APC_PUTS
    df_str = ap_consts.get_datafeed_str_ap(df_type=df_type)
    redis_db = ae_consts.REDIS_DB
    redis_key = f'{ticker}_{use_date}_apputs'
    redis_host, redis_port = ae_consts.get_redis_host_and_port(
        req=work_dict)
    redis_password = ae_consts.REDIS_PASSWORD
    s3_key = redis_key
    ...
    try:
        ...
        if status == ae_consts.SUCCESS:
            puts_json = None
            if 'apputs' in redis_rec['rec']['data']:
                puts_json = redis_rec['rec']['data']['apputs']
            ...
            try:
                ...
                if 'date' not in puts_df:
                    log.debug(
                        'failed to find date column in APC puts '
